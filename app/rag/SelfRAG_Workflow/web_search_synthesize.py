...
class WebSearchAgent:
    ...
    def _setup_tools(self):
        ...
        try:
            ...
            print(
                f"Tavily tools initialized: {len(self.tavily_tools)} tools available")
    ...
    def _setup_llm(self):
        """Setup the LLM (using AWS Bedrock)"""
        try:
            # Using AWS Bedrock with Meta Llama
            self.llm = Bedrock(
            ...
            print(f"Bedrock LLM initialized: {settings.bedrock_chat_model}")

        except Exception as e:
            print(f"Error setting up Bedrock LLM: {e}")
            raise Exception(f"Unable to initialize Bedrock LLM: {str(e)}")
    ...
    def web_search_synthesize(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            context = self._format_chat_history(
...
if __name__ == "__main__":
    ...
    try:
        ...
        result1 = web_agent.web_search_synthesize(
            "What are the latest developments in hollywood?")
