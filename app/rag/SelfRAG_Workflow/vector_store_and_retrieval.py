...
class SelfRAGVectorStore:
    def __init__(self, collection_name: str = "news_articles"):
        ...
        print(
            f"Initializing Bedrock embedding model: {settings.bedrock_embedding_model}")
    ...
    def build_vector_index(self, persist_directory: str = "./chroma_db"):
        ...
        try:
            ...
            if doc_count > 0:
                print(
                    f"Found existing vector store with {doc_count} documents. Loading existing index...")
    ...
    def retrieve_and_filter_nodes(self, query: str, similarity_threshold: float = 0.7):
        ...
        print(
            f"Retrieved {len(retrieved_nodes)} nodes, filtered to {len(filtered_nodes)} nodes with score >= {similarity_threshold}")
...
if __name__ == "__main__":
    ...
    filtered_nodes = rag_store.retrieve_and_filter_nodes(
