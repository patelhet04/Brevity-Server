...
class HallucinationCheckResult(BaseModel):
    ...
    confidence: float = Field(..., ge=0.0, le=1.0,
                              description="Confidence level of the assessment (0.0 to 1.0)")
    reasoning: str = Field(...,
                           description="Brief explanation of the decision")
    unsupported_claims: List[str] = Field(
        default=[], description="List of specific unsupported claims found")
...
class SelfRAGWorkflow(Workflow):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.hallucination_checker_llm = OpenAI(
            # You can change this to gpt-4, gpt-3.5-turbo, etc.
            model="gpt-4o-mini",
            temperature=0.1,                    # Low temperature for consistent checking
            max_tokens=512,
        ...
        logger.info(
            "✅ Self-RAG Workflow initialized with injected dependencies")
    ...
    async def retrieve_documents(self, ev: RetrieverEvent) -> QueryRouterEvent:
        ...
        try:
            ...
            logger.info(
                f"✅ Retrieved {len(retrieved_nodes)} relevant documents")
    ...
    async def query_router(self, ev: QueryRouterEvent) -> LLMSynthesizerEvent | WebSearchEvent:
        ...
        if num_documents >= 3:
            ev.retrieved_nodes = ev.retrieved_nodes[:2] if len(
                ev.retrieved_nodes) > 2 else ev.retrieved_nodes
...
Provide a comprehensive answer based primarily on the context provided. If the context doesn't fully answer the question, mention what information is available."""
            ...
            logger.info(
                f"☑️ Generated response, now checking for hallucinations...")
    ...
    async def hallucination_checker(self, ev: HallucinationCheckEvent) -> LLMSynthesizerEvent | StopEvent | WebSearchEvent:
        ...
        """
        logger.info(
            f"🕵️ Checking for hallucinations (attempt {ev.retry_count + 1})")
        ...
        try:
            ...
            facts_text = "\n".join([
                # Limit fact length for efficiency
                f"Fact {i+1}: {node.text}..."
                for i, node in enumerate(ev.retrieved_nodes)
            ])
...
JSON Response:"""
            ...
            try:
                ...
                if start_idx != -1 and end_idx > start_idx:
                    ...
                    check_result = HallucinationCheckResult(
                        is_hallucinated=parsed_result.get(
                            "is_hallucinated", False),
                        confidence=float(parsed_result.get("confidence", 0.5)),
                        reasoning=parsed_result.get(
                            "reasoning", "No reasoning provided"),
                        unsupported_claims=parsed_result.get(
                            "unsupported_claims", [])
                    )
                else:
                    raise json.JSONDecodeError(
                        "No valid JSON found", check_result_text, 0)

            except (json.JSONDecodeError, ValueError, TypeError) as e:
                logger.error(
                    f"‼️ Failed to parse hallucination check JSON: {e}")
            ...
            logger.info(
                f"ℹ️ Hallucination check result: {check_result.is_hallucinated} (confidence: {check_result.confidence})")
            logger.info(f"Reasoning: {check_result.reasoning}")
            if check_result.unsupported_claims:
                logger.info(
                    f"Unsupported claims: {check_result.unsupported_claims}")

            # Decision making
            if check_result.is_hallucinated and check_result.confidence > 0.7 and ev.retry_count < MAX_RETRIES:
                logger.info(
                    f"😵‍💫 Hallucination detected with high confidence. Retrying synthesis (attempt {ev.retry_count + 1}/{MAX_RETRIES})")
                ...
                if ev.retry_count >= MAX_RETRIES:
                    logger.info(
                        f"😵 Max retries ({MAX_RETRIES}) reached. Proceeding with web search...")
                    ...
                    logger.info(
                        "✅ Response appears to be grounded. Proceeding to final output.")

                # Update chat history
                updated_history = ev.chat_history + [
                    ChatMessage(role="user", content=ev.query),
                    ChatMessage(role="assistant",
                                content=ev.generated_response)
                ]
            ...
            logger.info(
                "Proceeding with original response due to checker error.")
    ...
    async def web_search_step(self, ev: WebSearchEvent) -> StopEvent:
        ...
        """
        logger.info(
            "🕸️ Performing web search due to insufficient retrieved context")
