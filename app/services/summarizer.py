...
def semantic_chunking(text, tokenizer, max_tokens=512):
    ...
    for chunk in chunks:
        ...
        if len(chunk_tokens) <= max_tokens:
            ...
            print(f"Warning: Chunk still too long ({len(chunk_tokens)} tokens), truncating...")
...
def efficient_semantic_chunking(text, tokenizer, max_tokens=512):
    ...
    for chunk in chunks:
        ...
        if len(chunk_tokens) > safe_max_tokens:
            print(f"Warning: Chunk still too long ({len(chunk_tokens)} tokens)")
...
async def map_reduce_summarization(article, summarizer, semaphore, max_tokens=1024):
    ...
    async with semaphore:
        try:
            ...
            combined_tokens = tokenizer.encode(combined_summary, add_special_tokens=True)
